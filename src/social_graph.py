...
# ========================【算法代码开始】========================
# 本模块实现社交网络图谱分析的核心算法，包括：
# 1. 人脉层级计算（可视化配色专用）
# 2. N度人脉统一查询接口（BFS实现）
# 3. 二度人脉择优路径查询（支持权重/兴趣双策略）
# 4. 无权图最短路径（BFS）+ 加权图最短路径（Dijkstra）
# 5. 基于兴趣的TopK智能推荐（自研小顶堆）
# 6. 社群分析（度中心性、连通分量）
# 7. 大数据性能测试工具
# 8. 数据导出工具
    ...
    def get_user_degree_layer(self, center_user_id: int) -> Dict[int, int]:
        ...
        功能说明：对应扩展功能E（可视化），为不同层级节点分配不同颜色
        :param center_user_id: 中心用户ID（可视化聚焦对象）
        :return: 字典 {用户ID: 层级数值}
        层级定义：
            0: 中心用户自己（红色）
            1: 一度人脉（直接好友）→ 橙色
            2: 二度人脉 → 黄色
            3: 其余所有节点 → 灰色（不限制深度，全部标记）
        自动跳过黑名单节点（不纳入遍历）
        时间复杂度：O(V+E)，遍历全图所有节点和边
        """
        # 校验中心用户是否存在
        if self.user_attrs.get(center_user_id) is None:
            return {}

        layer_map = defaultdict(int)   # 存储每个用户对应的层级
        visited = set()                # 已访问集合，避免重复遍历
        q = deque()                    # BFS队列
        q.append((center_user_id, 0))  # (用户ID, 当前深度)
        visited.add(center_user_id)
        layer_map[center_user_id] = 0

        # BFS广度优先遍历，限制最大深度为2（超过2度的统一归为灰色）
        while q:
            cur_uid, depth = q.popleft()
            # 超过2度不再深入遍历，避免不必要的队列操作
            if depth >= 2:
                continue
            # 获取当前用户的好友集合
    ...
    def find_second_degree_with_path(
        ...
        功能说明：基础功能4（二度人脉发现）的增强版本，增加了路径择优策略
        对应扩展功能B（加权图与优先推荐）
        :param user_id: 起始中心用户ID
        :param sort_strategy: 择优策略
            "weight": 按中间人好友总权重降序（亲密度最高中间人优先）
            "interest": 按中间人共同兴趣数量降序（兴趣契合优先）
            非法值自动降级为 "weight" 默认策略
        :return: 列表，每个元素为 (二度用户ID, 最优中间好友ID, 完整路径)
        示例：[(4, 2, [1, 2, 4]), (5, 3, [1, 3, 5])]
        时间复杂度：O(deg(user) * avg_deg(好友))，即一度好友数量 × 平均好友数
        """
        # 策略合法性校验，非法值降级为默认策略
    ...
    def find_n_degree_friends(self, user_id: int, n: int) -> List[int]:
        ...
        功能说明：对应基础功能3（一度人脉查询）和基础功能4（二度人脉发现）
        以及扩展功能A（多度人脉查询N≥3）
        设计原则：GUI层所有人脉查询统一调用本接口，不再手写BFS遍历逻辑
        严格遵循MVC分层架构
        :param user_id: 起始用户ID
        :param n: 人脉度数（1=一度好友，2=二度好友，3+为多度），必须为正整数
        :return: 升序排列的N度好友ID列表，自动过滤黑名单用户
        时间复杂度：O(deg^n)，BFS遍历到第n层
        """
        # 参数合法性校验
        ...
        q = deque()                        # BFS队列
        q.append((user_id, 0))             # (当前用户ID, 当前深度)
        visited.add(user_id)
        degree_result = defaultdict(list)  # 按深度存储节点：{深度: [用户ID列表]}

        # BFS广度优先遍历
        while q:
            cur_uid, depth = q.popleft()
            # 超过目标度数提前终止遍历（性能优化：剪枝）
            if depth >= n:
                continue
            # 获取当前用户的好友集合
            neighbor_set = self.graph.get(cur_uid)
    ...
    def get_shortest_distance(self, start_uid: int, end_uid: int) -> Tuple[int, List[int]]:
        ...
        功能说明：对应基础功能5（社交距离计算）的无权图模式
        :param start_uid: 起点用户ID
        :param end_uid: 终点用户ID
        :return: (距离值, 路径列表)
            - 距离值为边数（好友中转次数）
            - 路径列表为从起点到终点的用户ID序列
            - 无法连通返回 (-1, [])，目标在黑名单返回 (-1, [])
        时间复杂度：O(V+E)，遍历全图直到找到目标节点
        """
        # 基础合法性校验：起点和终点都必须存在
        ...
        # prev_node: 前驱节点字典，用于路径回溯
        # key=当前节点, value=前驱节点（从起点到该节点的路径中的上一个节点）
        ...
        while q:
            ...
            for neighbor in neighbor_set:
                ...
                q.append(neighbor)
                # 找到目标节点，清空队列提前退出（性能优化）
                if neighbor == end_uid:
                    q = deque()
                    break

        # 如果目标节点未被访问，说明不存在可达路径
        ...
        while temp is not None:
            ...
            if temp is not None and not isinstance(temp, int):
                break
        path.reverse()  # 反转得到从起点到终点的正序路径
        dist = len(path) - 1  # 距离 = 路径节点数 - 1
        return dist, path

    # ===================== 优化3：Dijkstra懒加载实现，移除全节点预初始化 =====================
    def get_weighted_shortest_path(self, start_uid: int, end_uid: int) -> Tuple[int, List[int]]:
        """
        Dijkstra迪杰斯特拉算法：惰性更新实现
        功能说明：对应基础功能5（社交距离计算）的加权图模式
        优化点：无需预先遍历所有用户初始化距离字典，大幅降低内存占用和初始化耗时
        适用场景：大规模社交网络（10000+节点）下的加权最短路径查询
        :param start_uid: 起点用户ID
        :param end_uid: 终点用户ID
        :return: (总权重和, 路径列表)
            - 总权重和为路径上所有边权重之和
        ...
        # 使用自研小顶堆（MinHeap）作为优先级队列
        heap = MinHeap()
        heap.push(0, start_uid)  # (当前累积权重, 节点ID)

        # Dijkstra主循环
        while heap.size() > 0:
            cur_weight, cur_uid = heap.pop()
            # 到达目标节点，提前终止（Dijkstra贪心特性保证首次弹出即为最短）
            if cur_uid == end_uid:
                break
            # 跳过堆中的过期记录（已找到更短路径的旧节点）
    ...
    def recommend_friends_by_interest(self, user_id: int, top_n: int = 5) -> List[Tuple[int, str, int, List[str]]]:
        ...
        功能说明：对应扩展功能C（基于兴趣的智能推荐）
        核心亮点：
            1. 使用自研MinHeap实现TopK选择，时间复杂度O(m log K)，m为候选用户数
            2. 返回具体的共同兴趣名称列表，满足"推荐理由展示"要求
            3. 自动排除：自身、已存在好友、黑名单用户
        :param user_id: 目标用户ID
        :param top_n: 推荐数量（默认5）
        :return: 列表，每个元素为 (用户ID, 用户名, 共同兴趣数量, 共同兴趣名称列表)
            按共同兴趣数量降序排列
        时间复杂度：O(m log K)，m为有共同兴趣的候选用户数，K=top_n
        """
        # 获取目标用户信息
        user_info = self.user_attrs.get(user_id)
        ...
        # 通过兴趣倒排索引收集候选用户及其匹配的兴趣列表
        # user_match_interests: {候选用户ID: [共同兴趣名称1, 共同兴趣名称2, ...]}
    ...
    def calc_degree_centrality(self) -> List[Tuple[int, int, str]]:
        """
        度中心性计算：统计每个用户的好友数量，找出社交网络的社群核心用户
        功能说明：可选的分析工具，用于识别社交网络中的关键人物
        :return: 列表，每个元素为 (用户ID, 好友数量, 用户名)
            按好友数量降序排列，好友数量相同时按ID升序
        时间复杂度：O(V)，遍历所有用户
        """
        centrality_list = []
        # 遍历哈希表中的所有用户
    ...
    def find_all_communities(self) -> List[List[int]]:
        """
        连通分量查找：划分社交网络中的所有独立社群（连通分量）
        功能说明：用于分析社交网络的群体结构
        :return: 列表，每个元素为一个社群（连通分量）的用户ID列表
            每个社群内的用户按ID升序排列，社群之间按最小ID升序排列
        时间复杂度：O(V+E)，遍历全图
        """
        visited = set()
        communities = []
                ...
                while q:
                    ...
                    for neighbor in neighbors:
                        if neighbor not in visited and neighbor not in self.blacklist:
                            visited.add(neighbor)
                            q.append(neighbor)
                one_community.sort()  # 内部按ID升序排列
                communities.append(one_community)

        # 按最小ID升序排列社群
        communities.sort(key=lambda x: x[0] if x else 0)
        return communities

    # ===================== 低优扩展B：加权混合推荐完整预留框架 =====================
    def recommend_friends_weight_mix(self, user_id: int, top_n: int = 5) -> List[Tuple[int, str, float, List[str]]]:
        """
        扩展功能B：结合好友亲密度权重 + 共同兴趣的混合评分推荐
        功能说明：对应扩展功能B（加权图与优先推荐）的混合推荐实现
        混合公式：综合得分 = 共同兴趣数 × 0.6 + 好友亲密度均值 × 0.4
        设计思路：同时考虑"兴趣契合度"和"社交亲密度"，推荐更精准
        :param user_id: 目标用户ID
        :param top_n: 推荐数量（默认5）
        :return: 列表，每个元素为 (用户ID, 用户名, 综合得分, 共同兴趣列表)
            按综合得分降序排列
        时间复杂度：O(m * deg(user))，m为候选用户数
        """
        user_info = self.user_attrs.get(user_id)
        if not user_info:
            return []
        ...
        for cand_uid, inters in match_dict.items():
            interest_score = len(inters)  # 兴趣维度得分

            # 计算亲密度维度得分：目标用户的好友中，有多少人也是候选用户的好友
            # 通过计算二度连接的平均权重来衡量社交亲密度
            total_weight = 0
            friend_cnt = 0
            for mid_friend in self.get_direct_friends(user_id):
                # 检查当前候选用户是否在中间好友的邻接表中
                # 注：这里的逻辑是简化的，实际应计算更复杂的亲密度指标
    ...
    def generate_big_test_data(self, user_num: int = 1000, edge_num: int = 5000):
        """
        随机生成大规模社交网络测试数据集
        功能说明：用于性能测试和扩展功能D（大数据量测试）的验证
        :param user_num: 用户总数（默认1000）
        :param edge_num: 好友边总数（默认5000）
        生成规则：
            - 每个用户随机分配2~4个兴趣标签（从10个预设标签中选取）
            - 随机生成无向好友边，确保不重复、不自环
            - 边权重随机1~10之间的整数
        """
        # 清空原有数据（完全重置，避免与历史数据混合）
    ...
    def run_performance_test(self, test_center_id: int = 1):
        """
        执行全套性能测试，打印各算法耗时
        功能说明：对应扩展功能D（性能优化）的性能评估工具
        测试项包括：
            1. 一度+二度人脉BFS查询
            2. BFS无权最短路径
            3. Dijkstra加权最短路径（惰性更新优化版）
            4. 择优二度人脉遍历
            5. 全网人脉层级计算
        :param test_center_id: 测试中心用户ID（默认1）
        """
        print("\n========== 大数据性能测试报告 ==========")
    ...
    def export_adjacency_list(self, filename: str) -> bool:
        """
        导出标准邻接表格式（用于图算法分析和可视化导入）
        功能说明：将社交网络图导出为标准邻接表格式，便于外部工具分析
        格式说明：每行 "用户ID 好友ID1 好友ID2 ..."
        示例： "1 2 3 6" 表示用户1的好友为2、3、6
    ...
    def export_adjacency_table_text(self, filename: str) -> bool:
        """
        导出纯文本带边框表格（人类可读格式）
        功能说明：导出带表格边框的文本文件，在记事本中也能整齐显示
        表格列：用户ID | 姓名 | 兴趣标签 | 好友列表(含姓名)
        格式特点：自动计算列宽，居中对齐，使用ASCII表格边框
        :param filename: 导出文件路径
        :return: 导出成功返回True，失败返回False
        """
        try:
            ...
            with open(filename, "w", encoding="utf-8") as f:
                ...
                for col_idx in range(len(header)):
                    ...
                    col_widths.append(max_width + 2)  # +2留出边距
                ...
                # 辅助函数：写入一行（好友列表列左对齐，其他居中）
                def write_row_left(cells):
                    row = "|"
                    for i, cell in enumerate(cells):
                        if i == 3:  # 好友列表列左对齐，方便阅读长列表
